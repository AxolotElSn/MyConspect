		...
		<div className={'mb-3 p-3 border border-' + props.color}>
			{/* {props.children} можно сказать что вместо этой конструкции появятся все элементы или компоненты переданные через пропсы */}
			{
				React.Children.map(props.children, child => {
					...
					return React.cloneElement(child, {className: 'shadow p-3 mx-auto m-3 border rounded'})
				})
			}
		</div>
	)
...
const HelloGreaning = () => { // мы получеется наследуем DynamicGreating и делаем его более специфичным это называется специализация или композиция. Она всегда используется
  return (
    <div style={{'width': '600px', 'margin': '0 auto'}}>
      	<DynamicGreating color={'primary'}>
			<h2 style={{'text-align': 'center'}}>.__. .-. .__.</h2>
		</DynamicGreating>
    </div>
  )
}
